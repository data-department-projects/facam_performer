...
import {
  ...
  ChevronFirst, ChevronLast, HelpCircle, BookOpen, AlertCircle
} from "lucide-react";
...
interface NavItem {
  ...
  adminOnly?: boolean;
  moduleId?: string;
  children?: {id: ViewType;label: string;icon: any;moduleId?: string;}[];
}
...
const AppSidebar = ({ currentView, onViewChange, onNavigateToAdminTab }: AppSidebarProps) => {
  const { isAdmin, allowedModules, signOut, profile, user } = useAuth();
  ...
  useEffect(() => {
    ...
    on(
      ...
      (payload) => {
        ...
        (payload.new as any)?.status === 'submitted' &&
        (payload.old as any)?.status !== 'submitted')
        {
          toast({
